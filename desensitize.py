...
SENSITIVE_TERMS: list[str] = [
    ...
    "zero-day",
    "0day",
    # 身份声明组合触发：后端会拦截「"agent 身份" + "OpenAI 出品" + 能力声明」三句
    # 同现的竞品 agent 系统提示（如 Codex CLI 开头），打断 OpenAI 一词即可解除组合。
    "OpenAI",
]
...
def _desensitize_description_fields(value):
    """Copy tool metadata while changing description text only."""
    if isinstance(value, list):
        return [_desensitize_description_fields(item) for item in value]
    ...
    for key, item in value.items():
        if key in {"description", "title"} and isinstance(item, str):
            result[key] = desensitize_text(item)
        else:
            result[key] = _desensitize_description_fields(item)
    return result


def desensitize_tools(tools: Iterable[dict]) -> list[dict]:
    """Process tool descriptions without changing names or JSON schemas."""
    return [_desensitize_description_fields(tool) for tool in tools]


def desensitize_body(
    body: dict,
    roles: tuple[str, ...] = ("system", "developer"),
    include_tools: bool = True,
) -> dict:
    """Process trusted client metadata while preserving user content."""
    nb = dict(body)
    if body.get("messages"):
        nb["messages"] = desensitize_messages(body["messages"], roles=roles)
    if include_tools and isinstance(body.get("tools"), list):
        nb["tools"] = desensitize_tools(body["tools"])
    return nb
...
if __name__ == "__main__":
    ...
    for s in samples:
        ...
        changed = "CHANGED" if d != s else "UNCHANGED"
        print(f"{changed} | 原文: {s}")
        if d != s:
            escaped = d.encode("unicode_escape").decode("ascii")
            print(f"     | 脱敏: {escaped}")
